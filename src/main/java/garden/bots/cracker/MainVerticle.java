...
public class MainVerticle extends AbstractVerticle {
  ...
  //? Route Handler
  private Handler<RoutingContext> defineHandler(Context extismContext, Manifest manifest, String wasmFunctionName) {
    ...
    return ctx -> {
      ...
      ctx.request().body().andThen(asyncRes -> {
        ...
        try {
          ...
          var plugin = extismContext.newPlugin(manifest, true);
  ...
  public void start(Promise<Void> startPromise) throws Exception {
    ...
    if (wasmRegistryDomain.isBlank()) {
      ...
      var handler = this.defineHandler(extismContext, manifest, wasmFunctionName);
